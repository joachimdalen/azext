...
type GithubIssueRequest = Endpoints['GET /repos/{owner}/{repo}/issues/{issue_number}']['response']
type GithubPrRequest = Endpoints['GET /repos/{owner}/{repo}/pulls/{pull_number}']['response']
...
class GitHub {
  ...
  async getIssues(id: number, repo: string): Promise<GitHubIssue | undefined> {
    ...
    try {
      ...
      if (err?.status) {
        if (err.status === 404) {
          console.log(chalk.redBright(`❌ Failed to find issue with number: ${id}`))
        }
      }
    }
  ...
  async getPullRequests(
    ...
    try {
      ...
      if (err?.status) {
        if (err.status === 404) {
          console.log(chalk.redBright(`Failed to find pull request with number: ${id}`))
        }
      }
    }
